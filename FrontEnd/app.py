# app.py — Interface Streamlit pour interroger l'agent IA Escalade
...
def load_data():
    ...
    # Nettoyage des colonnes nommées "Unnamed: 0" si présentes
    grades = grades.loc[:, ~grades.columns.str.contains('^Unnamed')]

    # --- TRADUCTION DES GRADES NUMÉRIQUES EN TEXTE (CSV) ---
    # Pour les voies : on arrondit la note moyenne pour obtenir un grade_id
    ...
    # Pour les grimpeurs : idem avec grades_mean
...
st.title("🧗 Assistant IA — Données d'escalade")

# ─────────────────────────────────────────────
# LLM (Ollama via interface OpenAI-compatible)
# ─────────────────────────────────────────────
...
with st.sidebar:
    ...
    # CORRECTION 1 : On force le filtre pays saisi en minuscules (.lower()) pour éviter les conflits de casse
...
def appliquer_filtres(nouveaux: dict):
    ...
    if nouveaux.get("country"):
        # CORRECTION 1 (suite) : Sécurité minuscule aussi via l'extraction LLM
...
with col1:
    ...
    # CORRECTION 2 & 3 : On garde la colonne 'grade_mean' et 'grade_fra' (le texte du CSV)
    map_df = (
    ...
    # On colore par 'grade_mean' (numérique pour le dégradé) et on ajoute 'grade_fra' dans le survol (hover)
    fig_map = px.scatter_mapbox(
        ...
        color="grade_mean",
        color_continuous_scale="Plasma", # Échelle du violet (facile) au jaune/jaune-fluo (dur)
        zoom=2,
        height=500,
        labels={"grade_mean": "Score Niveau", "grade_fra": "Grade Falaise"}
    )
...
# CORRECTION 3 : Groupement par 'grade_fra' (le texte du CSV) au lieu du chiffre brut
grade_country = (
    routes_visu
    .groupby(["country", "grade_fra", "grade_id_round"])
    .size()
    .reset_index(name="count")
    .sort_values("grade_id_round") # Permet de garder un tri logique du plus facile au plus dur dans la légende
)
...
fig_grade = px.bar(
    ...
    x="country",
    y="count",
    color="grade_fra", # Utilisation du grade textuel !
    barmode="stack",
    labels={
        "country": "Pays",
        "count": "Nombre de voies",
        "grade_fra": "Grade Français"
    }
)
...
# =====================================================
# Corrélation taille / poids / niveau
# =====================================================
...
fig_scatter = px.scatter(
    ...
    color="grades_mean",
    size="grades_mean",
    # CORRECTION 3 : Ajout du grade textuel (grade_fra) au survol de la souris
    hover_data={"country": True, "age": True, "years_cl": True, "grade_fra": True, "grades_mean": False},
    labels={
        "height": "Taille (cm)",
        "weight": "Poids (kg)",
        "grade_fra": "Grade Moyen"
    }
)
...
if question:
    ...
    st.caption(f"Filtres actifs : {st.session_state['filters']}")